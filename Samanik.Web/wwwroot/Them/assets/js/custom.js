...
function addToCart(id, name, price, picture) {
    //کوکی با این نام رو بگیر
    let products = $.cookie(cookieName);
    //اگر بدون مقداره یه مقدار آرایه خالی خودمون بهش میدیم
    if (products === undefined) {
        products = [];
    } else {
        //میخوایم محصول رو که استرینگ بوده به جبیسون تبدیلش کنیم
        products = JSON.parse(products);
    }

    //تعداد محصولمون رو میگیریم
    const count = $("#productCount").val();
    //توجه: حالا ما باید بررسی کنیم اگر این محصولی رو که ما داریم اضافه میکنیم
    //اگر قبلا داخل آرایه وجود داشت تعدادش رو افزایش بده ، درغیر اینصورت خودش رو اضافه کن
    //از داخل پروداکت پیداکن اونی که آیدیش با این آیدی که من فرستادم
    const currentProduct = products.find(x => x.id === id);
    //اگر وجود نداشت داخل محصولاتم
    if (currentProduct !== undefined) {
        //مجددا محصول رو پیدا میکنیبم و تعدادش رو برابر اونی میزاریم که وارد شده به اضه اون مقدار قبلی که داشت
        products.find(x => x.id === id).count = parseInt(currentProduct.count) + parseInt(count);
    } else {
        //اگرم وجود نداشت خودش رو اضافه کن
        const product = {
            id,
            name,
            unitPrice: price,
        ...
        //در نهایت محصولمون رو به اون آرایه اضافه میکنیم 
        products.push(product);
    }

    //و در نهایت این آرایه محصول رو به کوکی اضافه میکنیم
    //
