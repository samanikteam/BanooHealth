...
namespace Peygiry.Web.Pages.TagHelpers
{
    ...
    public class Pageing : TagHelper
    {
        ...
        public override void Process(TagHelperContext helperContext, TagHelperOutput helperOutput)
        {
            ...
            if (PagingModel.TotalPages > 0)
            {
                ...
                labelDiv.InnerHtml.Append($"نمایش {PagingModel.CurrentPage} از { PagingModel.TotalPages}");
                outerDiv.InnerHtml.AppendHtml(labelDiv);
                TagBuilder linkDiv = new TagBuilder("div");
                linkDiv.InnerHtml.AppendHtml(GeneratePageLinks("اول", 1));
                for (int i = startPage; i <= endPage; i++)
                {
                    linkDiv.InnerHtml.AppendHtml(GeneratePageLinks(i.ToString(), i));
                }

                linkDiv.InnerHtml.AppendHtml(GeneratePageLinks("آخر", PagingModel.TotalPages));
